...
# and set per platform
    ...
    def compile(self, node):
        ...
        if self.anm.has_overlay('vxlan') and node in self.anm['vxlan']:
            self.vxlan(node)
        if self.anm.has_overlay('mcast') and node in self.anm['mcast']:
            self.mcast(node)
        if self.anm.has_overlay('port_channel') and node in self.anm['port_channel']:
            self.port_channel(node)
    ...
    def port_channel(self, node):
        g_pc = self.anm['port_channel']
        g_phy = self.anm['phy']

        pc_node = g_pc.node(node)
        pc_stanza = node.add_stanza("vpc")
        #if 'vpc-peer' in pc_node and pc_node['vpc-peer'] is not None:
        if pc_node.vpc_peer is not None:
            #adding destination ip for vpc over here
            rand_no = random.randint(1,1000)
            for phy_node_dest in g_phy.l3devices():
                if phy_node_dest.id == pc_node.vpc_peer:
                    DmNode_dest = self.nidb.node(phy_node_dest)
                    node.vpc.dest = DmNode_dest.mgmt.ip
                    DmNode_dest.vpc.domain_id = rand_no
                    node.vpc.domain_id = rand_no
                    break
    ...
    def mcast(self, node):
        ...
        g_mcast = self.anm['mcast']

        mcast_node = g_mcast.node(node)
        mcast_stanza = node.add_stanza("mcast")
        node.pim = 1
    ...
    def vxlan(self, node):
        ...
        if 'ingress_replication' in vxlan_global_config and vxlan_global_config['ingress_replication'] == True:
        ...
        if vxlan_node.vxlan_vni_configured is not None:
            ...
            node.vxlan.svi=[]
            if 'anycast_gateway_mac' in vxlan_global_config and vxlan_global_config['anycast_gateway_mac'] is not None:
                node.vxlan.anycast_gateway_mac = vxlan_global_config['anycast_gateway_mac']
            for tenant in vxlan_node.vxlan_vni_configured:
                ...
                for l2_vni in l2_vnis_configured:
                    ...
                    if mcast_group is not None:
                        vni_mcast_stanza = ConfigStanza(vni=l2_vni, mcast_grp=mcast_group)
                    ...
                    if flag_first_time == True:
                        ...
                        if l3_vni_data is not None:
                            ...
                            vrf = l3_vni_data[2]
                            vrf_vni_vlan_stanza = ConfigStanza(vrf_context=vrf, vni=l3_vni, vlan=control_vlan)
                            node.vxlan.vrf_vni_vlan.append(vrf_vni_vlan_stanza)

                    if l3_vni is not None:
                        anycast_address = l2_vni_info[l2_vni][0]
                        vlan_svi_stanza = ConfigStanza(vlan=vlan, vrf_context=vrf, anycast_gateway_ip=anycast_address)
            ...
            if 'route_reflectors' in vxlan_global_config and vxlan_global_config['route_reflectors'] is not None:
                for rr in vxlan_global_config['route_reflectors']:
                    if vxlan_node.id == rr:
                       node.vxlan.rr = True
                       break
            if 'rendezvous_point' in vxlan_global_config and vxlan_global_config['rendezvous_point'] is not None:
                node.vxlan.rp=[]
                for rp in vxlan_global_config['rendezvous_point']:
                    rp_stanza = ConfigStanza(ip=rp['node_id'], mcastgrp=rp['mcast_group'])
    ...
    def ntp(self, node):
        ...
        if ntp_node.ntp is not None:
            node.ntp.enabled = ntp_node.ntp['enabled']
            node.ntp.server_ip = ntp_node.ntp['server_ip']
