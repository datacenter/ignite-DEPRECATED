...
% if node.snmp:
  ...
  % for server in node.snmp.server:
    snmp-server host ${server.ip} traps version ${server.version} ${server.community} udp_port ${server.udp_port}
  %endfor

  % for feature in node.snmp.features:
    snmp-server enable traps ${feature.id}
  %endfor
% endif
!
% if node.vpc:
...
interface ${interface.id}
  ...
  %endif
  % if node.vxlan.spine:
    % if interface.connected_to == 'leaf':
  ip router eigrp ${node.asn}
  ip pim sparse-mode
    %endif
  % endif
  ...
  ip pim sparse-mode
  % endif
  % if interface.id == 'Loopback101':
  ip router eigrp ${node.asn}
  ip pim sparse-mode
  % endif
  % if node.vxlan.leaf:
     % if interface.connected_to == 'spine':
  ip router eigrp ${node.asn}
  ip pim sparse-mode
     % endif
  % endif
