...
.directive('topologyViewer', ['appSettings', function (appSettings) {
    ...
    return {
        ...
        controller : function($scope) {
          ...
          $scope.prefix = $scope.prefix.substring(0,5);

          //Core
          for(var i=0; i<preFixedTopology.topology_json.core_list.length;i++){
              preFixedTopology.topology_json.core_list[i].name = $scope.prefix + '-' + preFixedTopology.topology_json.core_list[i].name;
          }

          //Spine
          for(var i=0; i<preFixedTopology.topology_json.spine_list.length;i++){
              preFixedTopology.topology_json.spine_list[i].name = $scope.prefix + '-' + preFixedTopology.topology_json.spine_list[i].name;
          }

          //Leaf
          for(var i=0; i<preFixedTopology.topology_json.leaf_list.length;i++){
              preFixedTopology.topology_json.leaf_list[i].name = $scope.prefix + '-' + preFixedTopology.topology_json.leaf_list[i].name;
          }

          //Host
          for(var i=0; i<preFixedTopology.topology_json.host_list.length;i++){
              preFixedTopology.topology_json.host_list[i].name = $scope.prefix + '-' + preFixedTopology.topology_json.host_list[i].name;
          }

          //Link List
          for(var i=0; i<preFixedTopology.topology_json.link_list.length;i++){
              preFixedTopology.topology_json.link_list[i].switch_1 = $scope.prefix + '-' + preFixedTopology.topology_json.link_list[i].switch_1;
              preFixedTopology.topology_json.link_list[i].switch_2 = $scope.prefix + '-' + preFixedTopology.topology_json.link_list[i].switch_2;
          }
